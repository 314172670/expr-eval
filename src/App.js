...
console.log(expr.evaluate({ x: 3 })," Parser.parse(expr).evaluate(vars)"); //(2*3+1) 7
// or
 // 42
console.log(Parser.evaluate('6 * x', { x: 7 }),"Parser.evaluate(expression: string, variables?: object)")
...
console.log(expr.evaluate({ x: 3 }),"创建新表达式代替参数substitute(variable: string, expression: Expression | string | number)")


//2.simplify(variables: object) 简化常量子表达式
expr = Parser.parse("x * (y * Atan(1))").simplify({ y: 4 }); //(x*(4*atan(1)))
console.log(expr.evaluate({ x: 2 }),"simplify")
...
console.log("x,y没有赋值",expr.variables())   //["x","y"]
console.log("x没有赋值",expr.simplify({ y: 4 }).variables()) //["x"]

//4.symbols(options?: object) 得到一组值，包含创建的函数名和参数名
//option接受一个参数，同上{ withMembers: true }包含对象成员

expr = Parser.parse("Min(x, y, z)");
console.log("Min(x, y, z)",expr.symbols()) //["min", "x", "y", "z"]
console.log("传入y、z",expr.simplify({ y: 4, z: 5 }).symbols())  //["min", "x"]
...
console.log("自定义函数customAddFunction",parser.evaluate('customAddFunction(2, 4)+2')); // true
...
//in 运算符只针对数组
console.log(parser.evaluate("'el' in str",{str:'Hello'}))  //true

// console.error(parser.evaluate("avg(a)",{a:[1,2,3,9]}))  //true
console.error(parser.evaluate("a || b",{a:1,b:2}))  //true




console.log(parser.evaluate("x == 1?'good':'fa'",{x:1}))  //'good'

console.log(parser.evaluate('2 ^ x', {x: 3})) //8

console.log(parser.evaluate('2^x.y', {x: {y: 3}})) //8

console.log(parser.evaluate('\'as\' || \'df\''))//asdf


console.log("1",Parser.evaluate('(x.z+2) + (y.a.b-1) ', {x:{z:5},y:{a:{b:10}}}) )
console.log("2",Parser.evaluate('(x.z+2) <= (y.a.b-1)? true:false', {x:{z:5},y:{a:{b:10}}}) )
console.log("3",Parser.evaluate('(x.z+2) <= (y.a.b-1)', {x:{z:5},y:{a:{b:7}}}) )

console.log("4",Parser.evaluate('Sign(x)', {x:9}))
